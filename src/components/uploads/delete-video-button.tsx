...
/**
 * Two-click delete for a video upload — first click asks for confirmation inline (no native
 * dialog), second click performs the delete. The server action enforces owner/admin, so this
 * button should only be rendered for people allowed to delete.
 */
export function DeleteVideoButton({
  ...
  const handleDelete = async () => {
    setLoading(true);
    const res = await deleteVideoUpload(uploadId);
    setLoading(false);
    if (res.error) {
